...
_target_database_name = "dl_newc_regular"
_target_table_name = "dbo_loansuffix_new"
_ref_table_name = "dbo_loansuffix"
...
display(flightsDF.select("id", "par").orderBy("id"))
...
# MAGIC %sql
# MAGIC ALTER TABLE dl_newc_regular.dbo_loansuffix_new CHANGE data_lake_integrate_time_utc after id 

# COMMAND ----------

spark.sql(f"OPTIMIZE {_target_database_name}.{_target_table_name} ZORDER BY id, data_lake_integrate_time_utc;")
...
display(spark.sql(f"select * from {_target_database_name}.{_target_table_name} where id = 9092635 "))

# COMMAND ----------

display(spark.sql(f"select * from {_target_database_name}.{_ref_table_name} where id = 9092635 "))
...
display(spark.sql(f"select * from {_target_database_name}.{_target_table_name} where guid >= 'D1F7CB6F-D204-41B4-9262-6AFA8E663DF3' "))

# COMMAND ----------

display(spark.sql(f"select * from {_target_database_name}.{_ref_table_name} where guid >= 'D1F7CB6F-D204-41B4-9262-6AFA8E663DF3' "))
