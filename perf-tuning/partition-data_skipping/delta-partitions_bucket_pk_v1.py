...
# MAGIC %sql
# MAGIC 
# MAGIC -- partitions post merge in delta table
# MAGIC select par,count(*) from dl_playground.perf_test_partitions_bucketby_primarykey group by par order by 1;
...
display(spark.sql(f"select count(*) from {_target_database_name}.{_target_table_name}"))

# COMMAND ----------

display(spark.sql(f"select id, * from {_target_database_name}.{_target_table_name}"))
...
# MAGIC %sql
# MAGIC 
# MAGIC select count(*) from dl_newc_regular.dbo_loansuffix

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC describe history  dl_newc_regular.dbo_loansuffix
